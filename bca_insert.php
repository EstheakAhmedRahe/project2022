...
$duplicate_mobile= mysqli_query($conn,"SELECT * FROM `bca_ragistration` WHERE mobile='$mobile'");
$duplicate_email= mysqli_query($conn,"SELECT * FROM `bca_ragistration` WHERE email='$email'");
...
if(mysqli_num_rows(
  ...
  echo"<script>location.href='bca_registration.php'</script>";
   ...
   $sql="INSERT INTO `bca_ragistration`( `name`, `email`,`mobile`,`nid`, `birth`,`experience`,`address`) VALUES ('$fullname','$email','$mobile','$nid','$birth','$experiance','$address')";
  ...
  ob_start();
    ...
    $pdf->Cell(0,5,'Basic Cricket Academy Sylhet',0,1,'C'); 
    $pdf->Cell(0,10,' ',0,1); 
