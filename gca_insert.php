...
$duplicate_mobile= mysqli_query($conn,"SELECT * FROM `gca_ragistration` WHERE mobile='$mobile'");
$duplicate_email= mysqli_query($conn,"SELECT * FROM `gca_ragistration` WHERE email='$email'");
...
if(mysqli_num_rows(
  $duplicate_mobile)){
    ...
    echo"<script>location.href='gca_registration.php'</script>";
  }
  ...
  echo"<script>location.href='gca_registration.php'</script>";
   ...
   $sql="INSERT INTO `gca_ragistration`( `name`, `email`,`mobile`,`nid`, `birth`,`experience`,`address`) VALUES ('$fullname','$email','$mobile','$nid','$birth','$experiance','$address')";
  ...
  ob_start();
    ...
    $pdf->Cell(0,5,'Green Sylhet Cricket Academy',0,1,'C'); 
    $pdf->Cell(0,10,' ',0,1); 
